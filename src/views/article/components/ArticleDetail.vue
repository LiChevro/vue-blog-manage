            ...
            <div class="postInfo-container">
              <el-row>
                ...
                <el-col :span="10">
                  <el-form-item
                    label-width="40px"
                    :label="$t('article.catalog')"
                    class="postInfo-container-item"
                  >
                    <el-select
                      v-model="postForm.author"
                      :remote-method="getRemoteCatList"
                      filterable
                      default-first-option
                      remote
                      :placeholder="$t('article.catalog_placeholder')"
                      style="width: 300px;"
                    >
                      <el-option
                ...
                <el-col :span="10">
                  ...
                  >
                    <el-select
                      ...
                      multiple
                      allow-create
                      :placeholder="$t('article.tag_placeholder')"
                      style="width: 300px;"
                    >
                      <el-option
...
import { getTags, getCats, getArticle, defaultArticleData } from '@/api/articles'
...
export default class extends Vue {
    ...
    private tagListOptions = [];
    private catListOptions = [];
    ...
    private async getRemoteCatList(name: string) {
      const { data } = await getCats({ name })
      if (!data.items) return
      this.catListOptions = data.items.map((v: any) => v.name)
    }

    private async getRemoteTagList(name: string) {
      const { data } = await getTags({ name })
      if (!data.items) return
      this.tagListOptions = data.items.map((v: any) => v.name)
    }
}
</script>
